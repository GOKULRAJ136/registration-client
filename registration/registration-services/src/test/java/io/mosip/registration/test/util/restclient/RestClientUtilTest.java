...
public class RestClientUtilTest {
    ...
    public void invokeURL_returnsBodyAndHeaders() throws Exception {
        ...
        // Provide a non-null HttpEntity so Mockito stub matches
    ...
    public void downloadFile_savesSignatureAndSupportsResume() throws Exception {
        // prepare temp file using java.io.File to avoid JDK module reflective issues with PowerMock
        ...
        // setup resume: pretend we have a recorded larger content length
        ...
        // create partial file by appending 4 more bytes to make length 8
        ...
        // second execute simulates server indicating total length and same signature
        doAnswer(invocation -> {
            ClientHttpResponse resp = mock(ClientHttpResponse.class);
            HttpHeaders headers = new HttpHeaders();
            headers.add("file-signature", "sig1");
