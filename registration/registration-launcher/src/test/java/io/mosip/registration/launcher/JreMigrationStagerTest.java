...
public class JreMigrationStagerTest {
    ...
    private TestServer baseSetup(File root) throws Exception {
        ...
        return new TestServer(serve(routes), rootManifestFile);
    }
    ...
    public void stage_invalidLibSignature_aborts() throws Exception {
        ...
        try {
            JreMigrationStager.stage(root, rootManifestFile,
                    url(server, "/v/lib/MANIFEST.MF"), url(server, "/v/lib/MANIFEST.MF.sig"), url(server, "/v/lib.zip"),
                    keyPair.getPublic(), 50000, 0);
            fail("expected IOException when lib signature is invalid");
        } catch (IOException expected) {
            assertTrue(expected.getMessage().contains("lib staging failed"));
        } finally {
            server.stop(0);
        }
    }
    ...
    private static final class TestServer {
        ...
        final File rootManifest;
        TestServer(HttpServer server, File rootManifest) {
            this.server = server;
