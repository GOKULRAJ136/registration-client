...
public class MigrationCleanerTest {
    ...
    @Test
    public void cleanup_removesAllPresentArtefacts() throws Exception {
    ...
    @Test
    public void cleanup_noArtefacts_returnsEmpty() {
        List<String> removed = MigrationCleaner.cleanup(folder.getRoot());
        assertTrue(removed.isEmpty());
    }

    @Test
    public void cleanup_onlySomeArtefacts_removesOnlyThose() throws Exception {
        File base = folder.getRoot();
        Files.write(new File(base, "migration.exe").toPath(), "x".getBytes());
