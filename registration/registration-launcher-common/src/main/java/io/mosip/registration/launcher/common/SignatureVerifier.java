...
public final class SignatureVerifier {
    ...
    /**
     * Convenience overload that verifies a detached signature file against a data file using the
     * public key from the supplied certificate stream.
     *
     * @param dataFile          the signed file (e.g. {@code MANIFEST.MF})
     * @param signatureFile     the detached signature file (e.g. {@code MANIFEST.MF.sig})
     * @param certificateStream X.509 certificate providing the trusted public key
     * @return {@code true} only if the signature verifies
     * @throws IOException              if a file cannot be read
     * @throws GeneralSecurityException if the certificate is invalid
     */
    public static boolean verifyDetached(File dataFile, File signatureFile, InputStream certificateStream)
            throws IOException, GeneralSecurityException {
        byte[] data = Files.readAllBytes(dataFile.toPath());
        byte[] signature = Files.readAllBytes(signatureFile.toPath());
        return verify(data, signature, loadPublicKeyFromCertificate(certificateStream));
    }
}
